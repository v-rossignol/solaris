...
describe('getRelocateErrorMessage', () => {
  ...
  it('returns a 403 fallback when the server sends no message', () => {
    expect(getRelocateErrorMessage(createAxiosError(403))).toBe(
      'You are not allowed to travel to this star system.',
    );
  });

  it('returns a 404 fallback when the server sends no message', () => {
    expect(getRelocateErrorMessage(createAxiosError(404))).toBe('This star system was not found.');
  });
